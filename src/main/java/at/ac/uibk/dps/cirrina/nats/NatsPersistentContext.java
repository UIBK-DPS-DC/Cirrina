 ...
 */
public final class NatsPersistentContext extends InMemoryContext implements KeyValueWatcher, AutoCloseable {
    /**
    ...
    public NatsPersistentContext(String natsUrl) throws CoreException {
        ...
        try {
            ...
            throw new CoreException(String.format("Could not connect to the NATS server: %s", e.getCause()));
        }
        ...
        try {
            ...
            throw new CoreException("Failed to retrieve the persistent context bucket, make sure that it has been created");
        }
    ...
    public ContextVariable create(String name, Object value) throws CoreException {
        try (var bs = new ByteArrayOutputStream(); var os = new ObjectOutputStream(bs)) {
            ...
            throw new CoreException(String.format("Failed to create variable '%s': %s", name, e.getCause()));
        }
    }
    ...
    public void watch(KeyValueEntry keyValueEntry) {
        ...
        if (variables.containsKey(variableName)) {
            ...
            try (ByteArrayInputStream bis = new ByteArrayInputStream(bytes); ObjectInputStream ois = new ObjectInputStream(bis)) {
                ...
                throw new RuntimeException(String.format("Failed to update variable '%s': %s", variable.name, e.getCause()));
            }
        }
    }
    ...
    public void close() throws Exception {
        ...
        for (var variable : getAll()) {
            try {
                ...
                throw new CoreException(String.format("Failed to remove variable '%s': %s", variable.name, e.getCause()));
            }
        }
        ...
        try {
            ...
            throw new CoreException(String.format("Failed to close the connection to the NATS server: %s", e.getCause()));
        }
    }
}
